      ...
      <header className="bg-primary text-white px-6 py-4">
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div>
            ...
            <p className="text-sm" style={{ color: '#3D5A80' }}>SENTINEL Onboarding</p>
          </div>
          {clients.length > 0 && (
      ...
      <main id="main-content" className="px-6 py-6 max-w-7xl mx-auto">
        ...
        <Routes>
          ...
          <Route
            path="/audit"
            element={
              <AuditDashboard clients={clients} complianceRecords={complianceRecords} />
            }
          />
          <Route
            path="/findings"
            element={<FindingsPanel clients={clients} />}
          />
        </Routes>
      </main>
    </>
  ...
  return (
    <div className="py-16 text-center">
      ...
      <p className="text-neutral text-sm">
        Place <code className="font-mono bg-background px-1 py-0.5 rounded">client_onboarding.csv</code> in the{' '}
        <code className="font-mono bg-background px-1 py-0.5 rounded">public/</code> folder and refresh.
      </p>
    </div>
  )
}
