...
describe('nullsLastComparator', () => {
  it('sorts non-null strings ascending', () => {
    const items: { name: string | null }[] = [{ name: 'Zurich' }, { name: 'Mayfair' }, { name: 'Aberdeen' }]
    const comparator = nullsLastComparator((item: { name: string | null }) => item.name, 'asc')
  ...
  it('sorts non-null strings descending', () => {
    const items: { name: string | null }[] = [{ name: 'Zurich' }, { name: 'Mayfair' }, { name: 'Aberdeen' }]
    const comparator = nullsLastComparator((item: { name: string | null }) => item.name, 'desc')
    const sorted = [...items].sort(comparator)
    expect(sorted.map((item) => item.name)).toEqual(['Zurich', 'Mayfair', 'Aberdeen'])
  })

  it('places null values last in ascending order', () => {
    const items: { name: string | null }[] = [{ name: null }, { name: 'Mayfair' }, { name: 'Aberdeen' }]
    const comparator = nullsLastComparator((item: { name: string | null }) => item.name, 'asc')
    const sorted = [...items].sort(comparator)
  ...
  it('places null values last in descending order', () => {
    const items: { name: string | null }[] = [{ name: 'Mayfair' }, { name: null }, { name: 'Aberdeen' }]
    const comparator = nullsLastComparator((item: { name: string | null }) => item.name, 'desc')
    const sorted = [...items].sort(comparator)
  ...
  it('places multiple nulls last', () => {
    const items: { name: string | null }[] = [{ name: null }, { name: 'Mayfair' }, { name: null }, { name: 'Aberdeen' }]
    const comparator = nullsLastComparator((item: { name: string | null }) => item.name, 'asc')
    const sorted = [...items].sort(comparator)
  ...
  it('sorts numbers ascending with nulls last', () => {
    const items: { value: number | null }[] = [{ value: 500 }, { value: null }, { value: 100 }, { value: null }, { value: 300 }]
    const comparator = nullsLastComparator((item: { value: number | null }) => item.value, 'asc')
    const sorted = [...items].sort(comparator)
    expect(sorted.map((item) => item.value)).toEqual([100, 300, 500, null, null])
  })

  it('sorts numbers descending with nulls last', () => {
    const items: { value: number | null }[] = [{ value: 500 }, { value: null }, { value: 100 }]
    const comparator = nullsLastComparator((item: { value: number | null }) => item.value, 'desc')
    const sorted = [...items].sort(comparator)
    expect(sorted.map((item) => item.value)).toEqual([500, 100, null])
  })

  it('treats two nulls as equal (stable)', () => {
    const items: { id: number; name: string | null }[] = [{ id: 1, name: null }, { id: 2, name: null }]
    const comparator = nullsLastComparator((item: { id: number; name: string | null }) => item.name, 'asc')
    const sorted = [...items].sort(comparator)
